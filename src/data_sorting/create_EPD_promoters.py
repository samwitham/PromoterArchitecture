...
parser.add_argument('EPD_promoters_5UTR', type=str, help='Output location of the EPD_promoters5UTR bedfile')
...
parser.add_argument('promoter_length', type=int, help='Length of promoter to use upstream of EPD TSS in second promoter-5UTR bedfile')
parser.add_argument('fiveUTR_length', type=int, help='Length of 5UTR to use downstream of EPD TSS in second promoter-5UTR bedfile')
parser.add_argument('flagged_EPD_overlapping_specified_length', type=str, help='Output location of flagged EPD_promoters_5UTR which extend into coding regions')
args = parser.parse_args()

def create_EPD_proms(promoter_5UTR_bedfile,EPD_TSS_bed,EPD_promoters,flagged_proms_not_in_EPD,flagged_EPD_TSS_in_CDS,flagged_EPD_overlappingprom_so_only5UTRs,EPD_promoters_5UTR,promoter_length,fiveUTR_length,flagged_EPD_overlapping_specified_length):
    ...
    for i,data in EPD_proms.iterrows():
        ...
        if EPD_proms.loc[i,'strand'] == '+':            
            merged.loc[i,'newstart_EPDprom'] = EPD_proms.loc[i,'stop_EPDprom'] - promoter_length
            merged.loc[i,'newstop_EPDprom'] = EPD_proms.loc[i,'stop_EPDprom'] + fiveUTR_length
        ...
        if EPD_proms.loc[i,'strand'] == '-':
            merged.loc[i,'newstart_EPDprom'] = EPD_proms.loc[i,'start_EPDprom'] - fiveUTR_length
            merged.loc[i,'newstop_EPDprom'] = EPD_proms.loc[i,'start_EPDprom'] + promoter_length
    #make start and stop integars
    merged = merged.astype({'newstart_EPDprom': 'int','newstop_EPDprom':'int'})
    
    #sort on chr and start
    merged.sort_values(['chr','newstart_EPDprom'], inplace=True, ignore_index=True)
    
    #select correct columns
    EPD_proms_shortened = merged[['chr','newstart_EPDprom','newstop_EPDprom','AGI','dot','strand','source','type','dot2','attributes']].copy()    
    
    #flag and remove genes where the promoter start or 5UTR end are beyond the original Araport11 promoters_5'UTR file
    merged2 = pd.merge(EPD_proms_shortened,promoter_5UTRs, on='AGI', how='left',suffixes=('','_ara'))
    
    #make positive and negative strand masks
    pos_mask = merged2[merged2.strand=='+']
    neg_mask = merged2[merged2.strand=='-']
    #make dfs for pos and neg strands where the EPD TSS is in a coding region
    pos_in_CDS = pos_mask[(pos_mask.stop < pos_mask.newstop_EPDprom) | (pos_mask.start > pos_mask.newstart_EPDprom)]
    neg_in_CDS = neg_mask[(neg_mask.start > neg_mask.newstart_EPDprom) | (neg_mask.stop < neg_mask.newstop_EPDprom)]
    #concatenate those dfs
    extends_coding = pd.concat([pos_in_CDS,neg_in_CDS],ignore_index=True)
    #filter columns
    extends_coding = extends_coding[['chr','newstart_EPDprom','newstop_EPDprom','AGI','dot','strand','source','type','dot2','attributes']]
    #sort on chr and start
    extends_coding.sort_values(['chr','newstart_EPDprom'], inplace=True, ignore_index=True)
    #save file with flagged genes then overlap a coding region
    BedTool.from_dataframe(extends_coding).saveas(flagged_EPD_overlapping_specified_length)
    
    #shorten the EPD promoter/5UTRs if they are flagged
    merged_again = pd.merge(EPD_proms_shortened, merged_orig, on='AGI',how='left',suffixes=('','_ara'))
    ...
    for i,data in EPD_proms_again.iterrows():       
        ...
        if EPD_proms_again.loc[i, 'start'] > EPD_proms_again.loc[i, 'newstart_EPDprom']:
            merged_again.loc[i,'newnewstart_EPDprom'] = EPD_proms_again.loc[i, 'start']
            ...
            merged_again.loc[i,'newnewstop_EPDprom'] = EPD_proms_again.loc[i, 'stop']
...
#             merged_again.loc[i,'newnewstop_EPDprom'] = EPD_proms_again.loc[i, 'newstop_EPDprom']
    ...
    #make start and stop integars
    merged_again = merged_again.astype({'newnewstart_EPDprom': 'int','newnewstop_EPDprom':'int'})
    #sort on chr and start
    merged_again.sort_values(['chr','newnewstart_EPDprom'], inplace=True, ignore_index=True)
    #find lengths of promoters/5UTRs
    merged_again['length'] = merged_again.newnewstop_EPDprom - merged_again.newnewstart_EPDprom
    ...
    #select columns
    EPD_proms_shortened = merged_again[['chr','newnewstart_EPDprom','newnewstop_EPDprom','AGI','dot','strand','source','type','dot2','attributes']]
...
create_EPD_proms(args.promoter_5UTR_bedfile,args.EPD_TSS_bed,args.EPD_promoters,args.flagged_proms_not_in_EPD,args.flagged_EPD_TSS_in_CDS,args.flagged_EPD_overlappingprom_so_only5UTRs,args.EPD_promoters_5UTR,args.promoter_length,args.fiveUTR_length,args.flagged_EPD_overlapping_specified_length)