...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description="filter_microarray_conditions"
    )

    parser.add_argument(
        "input_file",
        type=str,
        help="Input location of expression data table (schmid et al 2005 microarray AtGE_dev_gcRMA.txt.newline)",
    )
    parser.add_argument(
        "output_file", type=str, help="Output location of filtered microarray"
    )
    parser.add_argument(
        "output_tau", type=str, help="Output location of TAU table"
    )
    parser.add_argument(
        "file_names",
        type=str,
...
def calculate_tau(expression_data, output_tau, file_names):
    ...
    dirName = f"../../data/output/{file_names}/genes/tissue_specific"
    try:
    ...
    df_filtered2 = df_filtered[
        ~df_filtered.Affy_identifier.str.startswith("AFFX")
    ]
    ...
    for ID in list_of_conditions:
        ...
        df_filtered2 = pd.merge(
            df_filtered2, tempdf, how="left", on="AGI code"
        )
...
def main(args):
    ...
    calculate_tau(expression_data, args.output_tau, args.file_names)
