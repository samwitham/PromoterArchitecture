...
def parse_args(args):
    ...
    parser.add_argument(
        "Czechowski_rankedcv",
        type=str,
        help="Input location of Czechowski et al 2005 ranked cv dataset reanalysed by Will Nash",
    )
    parser.add_argument(
        "Mergner_rankedcv",
        type=str,
        help="Input location of Mergner et al 2020 ranked cv dataset",
    )
    parser.add_argument(
        "no_of_genes",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        help="Output location of microarray gene category subsets",
    )
    parser.add_argument(
        "Mergner_gene_categories",
        type=str,
        help="Output location of RNAseq gene category subsets",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "Mergner_allgenes",
        type=str,
        help="Output location of all filtered RNAseq genes",
    )
    parser.add_argument(
        "promoters_gff3",
        type=str,
...
def filter_genes_czechowski(
    promoter_bed, select_genes_file, Czechowski_allgenes
):
    ...
    # save df
    filtered_df.to_csv(
        Czechowski_allgenes, sep="\t", columns=filtered_df.columns, index=False
    )
...
def filter_genes_mergner(promoter_bed, select_genes_file, Mergner_allgenes):
    ...
    # save df
    filtered_df.to_csv(
        Mergner_allgenes, sep="\t", columns=filtered_df.columns, index=False
    )
...
def main(args):
    ...
    filtered_czechowski = filter_genes_czechowski(
        args.promoters_filtered_contain_motifs,
        args.Czechowski_rankedcv,
        args.Czechowski_allgenes,
    )
    filtered_mergner = filter_genes_mergner(
        args.promoters_filtered_contain_motifs,
        args.Mergner_rankedcv,
        args.Mergner_allgenes,
    )
    # czechowksi subset
    subSet_onCV(
        filtered_czechowski, args.Czechowski_gene_categories, args.no_of_genes
    )
    # mergner subset
    subSet_onCV(
        filtered_mergner, args.Mergner_gene_categories, args.no_of_genes
    )
