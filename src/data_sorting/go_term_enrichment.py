...
def generate_background(
    background_gene_set, NCBI_gene_list, NCBI_gene_list_filtered
):
    ...
    NCBI_list_AGI = NCBI_list.assign(
        AGI=NCBI_list.Aliases.str.extract(r"(.*?)\,")
    )
    ...
    NCBI_list_filtered = NCBI_list_AGI[
        NCBI_list_AGI.AGI.isin(background_df.AGI)
    ]
    # reset  index
    NCBI_list_filtered.reset_index(inplace=True, drop=True)

    # remove AGI column
    NCBI_list_filtered = NCBI_list_filtered[
        [
            "tax_id",
            "Org_name",
            "GeneID",
    ...
    # save file
    NCBI_list_filtered.to_csv(
        NCBI_gene_list_filtered, sep="\t", header=1, index=None
    )
    ...
    NCBIgeneToPythonCli().tsv_to_py(
        NCBI_gene_list_filtered, "genes_ncbi_3702_proteincoding.py"
    )
...
def load_files(obo_fname, fin_gene2go):
    ...
    for nspc, id2gos in ns2assoc.items():
        print(
            "{NS} {N:,} annotated Arabidopsis genes".format(
...
def map_AGI2NCBI(NCBI_gene_list_filtered, genes_of_interest):
    ...
    genes_of_interest_df = pd.read_table(
        genes_of_interest, sep="\t", header=None
    )
    cols = ["AGI", "gene_type"]
    ...
    # merge the dfs
    merged = pd.merge(
        genes_of_interest_df, NCBI_list_AGI, how="left", on="AGI"
    )
...
def run_GOEA(study_genes, gene_class, goeaobj):
    ...
    # plot results
    plot_results(
        f"{gene_class}" + "_sig_enrichment_{NS}.png", goea_results_sig
    )
...
def main(args):
    ...
    generate_background(
        args.background_gene_set, args.NCBI_gene_list, NCBI_gene_list_filtered
    )
    ...
    constitutive_geneid2symbol = dict(
        zip(constitutive_genes.GeneID, constitutive_genes.Symbol)
    )
    variable_geneid2symbol = dict(
        zip(variable_genes.GeneID, variable_genes.Symbol)
    )
