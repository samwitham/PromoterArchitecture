...
parser.add_argument('Czechowski_rankedcv', type=str, help='Input location of all genes ranked by CV')
parser.add_argument('Czechowski_gene_categories_tfs', type=str, help='Output location of the TF gene categories')
parser.add_argument('Czechowski_all_tfs', type=str, help='Output location of all ranked TFs by CV value')
...
def filter_genes_czechowski(TF_list, select_genes_file):
    ...
    #save df
    filtered_df.to_csv(args.Czechowski_all_tfs,sep='\t',columns=filtered_df.columns, index=False)
...
filtered_czechowski = filter_genes_czechowski(args.TF_list,args.Czechowski_rankedcv)
#filtered_mergner = filter_genes_mergner(args.promoters_filtered_contain_motifs,args.Mergner_rankedcv)
#czechowksi subset
subSet_onCV(filtered_czechowski,args.Czechowski_gene_categories_tfs,args.no_of_genes)
#mergner subset
