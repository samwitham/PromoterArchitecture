...
parser.add_argument('TF_list', type=str, help='List of Arabidopsis transcription factors')
parser.add_argument('Schmid_allgenes', type=str, help='Input location of all filtered microarray genes ranked by tau')
parser.add_argument('Schmid_gene_categories_tfs', type=str, help='Output location of TF gene categories')
parser.add_argument('CV_TF_gene_categories', type=str, help='Input location of Czechowski coefficient of variation gene categories')
...
parser.add_argument('Schmid_all_tfs', type=str, help='Output location of all ranked TFs by tau value')
args = parser.parse_args()

def filter_genes_schmid(TF_list, select_genes_file):
   ...
   #sort by TF ID
    ...
    #save df
    filtered_df.to_csv(args.Schmid_all_tfs,sep='\t',columns=filtered_df.columns, index=False)
...
def subSet_ontau(in_df, out_dir, no_of_genes, CV_gene_categories):
    ...
    output_set = pd.concat([nonspecific[['AGI', 'state']], tissue_specific[['AGI', 'state']], samples_from_bins[['AGI', 'state']]], ignore_index = True)
...
filtered_schmid = filter_genes_schmid(args.TF_list,args.Schmid_allgenes)
subSet_ontau(filtered_schmid,args.Schmid_gene_categories_tfs,args.no_of_genes,args.CV_TF_gene_categories)