...
def read_in_files(input_folder, guide_pairs, gene):
    ...
    mutations_df = pd.read_table(
        f"{input_folder}{gene}_TFBSoverlapping.tsv", sep="\t", header=None
    )
    # add columns
    ...
    # #save file
    mutations_df.to_csv(
        f"{input_folder}{gene}_TFBSoverlapping.tsv",
        sep="\t",
        index=False,
        header=True,
    )
...
def main(args):
    ...
    # read in files
    mutations_df, guide_pairs_df = read_in_files(
        args.input_folder, args.guide_pairs, args.gene_name
    )
    # read_in_files(args.input_folder,args.guide_pairs,args.gene_name)

    # #genotype the plant lines, producing an output tsv
    mutations_df_genotyped = genotype_plant_lines(
        mutations_df, args.output_folder, args.gene_name
    )
