...
def flatten_mutations(mutations_df,output_folder,gene_name):
    ...
    one_row_each = mutations_df.plant_ID.drop_duplicates(keep='first').to_frame()
    #turn into a dict
    ...
    for row_dict in one_row_each_dict:
        ...
        for row in filtered_dict:
            ...
            if 'insertion' in mutation_type:
                ...
                #convert genomic positions from string to list
                insertion_positions_relative_to_TSS = row['insertion_positions_relative_to_TSS'].strip('][').split(', ')
            ...
            if 'deletion' in mutation_type:
                ...
                #convert genomic positions from string to list
                deletion_positions_relative_to_TSS = row['deletion_positions_relative_to_TSS'].strip('][').split(', ')
            ...
            if 'substitution' in mutation_type:
                ...
                #convert genomic positions from string to list
                substitution_positions_relative_to_TSS = row['substitution_positions_relative_to_TSS'].strip('][').split(', ')
        ...
        if insertion_genomic_positions_combined == []:
            ...
            dd['insertion_genomic_positions_combined']= insertion_genomic_positions_combined
            ...
            #remove duplicates
            insertions.sort()
            insertions_no_dups = list(insertions for insertions,_ in itertools.groupby(insertions))
            #make integers [[int(float(j)) for j in i] for i in x]
            insertions_no_dups = [[int(float(j)) for j in i] for i in insertions_no_dups]
        ...
        if insertion_positions_relative_to_TSS_combined == []:
            ...
            dd['insertion_positions_relative_to_TSS_combined']= insertion_positions_relative_to_TSS_combined
        else:
            #make integers
        ...
        if deletion_genomic_positions_combined == []:
            ...
            dd['deletion_genomic_positions_combined']= deletion_genomic_positions_combined
            dd['deletion_sizes']= deletion_sizes
        else:
            #make integers
            deletions = [int(l) for l in list(np.unique(deletion_genomic_positions_combined))] 
        ...
        if substitution_genomic_positions_combined == []:
            ...
            dd['substitution_genomic_positions_combined']= substitution_genomic_positions_combined
            dd['substitution_sizes']= substitution_sizes
        else:
            #make integers
            substitutions = [int(l) for l in list(np.unique(substitution_genomic_positions_combined))] 
        ...
        if substitution_positions_relative_to_TSS_combined == []:
            ...
            dd['substitution_positions_relative_to_TSS_combined']= substitution_positions_relative_to_TSS_combined
        else:
            #make integers
        ...
        if len(mutation_types_unique)==1:
            ...
            #make unique
            mutation_types_unique_new = list(np.unique(mutation_types_unique_new))
    ...
    df_single_rows['ID_number'] = df_single_rows.plant_ID.str.split('-').str[1].astype(int)
    ...
    #save this df
    df_single_rows.to_csv(f'{output_folder}/{gene_name}_TFBSoverlapping_genotyped_only_mutated_flattened.tsv', sep="\t", index=False, header=1)
