...
def select_guides(input_file, region_start, region_end,lower_bound_distance, upper_bound_distance):
    ...
    for i, row in merged_copy.iterrows():
        ...
        if any(guide1 == v for v in dict1.values()) or any(guide2 == v for v in dict2.values()):            
            pass
        #if guide 1 is in column 2 and guide 2 is not in column1, swap places with guide 2
        elif any(guide1 == v for v in dict2.values()) and any(guide2 != v for v in dict1.values()):
        ...
        if any(guide2 == v for v in dict2.values()) or any(guide1 == v for v in dict1.values()):
            pass
        #if guide 2 is in column 1 and guide 1 is not in column 2, swap guide 1 and guide 2
        elif any(guide2 == v for v in dict1.values()) and any(guide1 != v for v in dict2.values()):
            merged.loc[i,'guide1'] = guide2
            merged.loc[i,'guide2'] = guide1
        #record guidenames again incase they switched
        guide1 = merged.loc[i,'guide1']
...
def adapt_primers(gene_info_df,gene_name, output_file):
    ...
    for i,data in gene_info_df.iterrows():
        ...
        if gene_info_df.loc[i,'sequence'][0] != 'G' and gene_info_df.loc[i,'sequence'][0] != 'g':
            gene_info_df.loc[i, 'new_sequence'] = 'g' + gene_info_df.loc[i,'sequence']
        else:
            #else use the original sequence
            gene_info_df.loc[i, 'new_sequence'] = gene_info_df.loc[i,'sequence']
        #add the BsaI to the start of the sequence
        gene_info_df.loc[i, 'new_sequence'] = 'tgtGGTCTCtatt' + gene_info_df.loc[i, 'new_sequence']
        ...
        gene_info_df.loc[i, 'new_sequence'] = gene_info_df.loc[i, 'new_sequence'] + 'gtttaagagctatgctggaaac'
        ...
        gene_info_df.loc[i, 'length'] = len(gene_info_df.loc[i, 'new_sequence'])
        #add new column containing gene name and the new guide number
        gene_info_df.loc[i, 'gene_guidename'] = gene_name + '_'+ gene_info_df.loc[i,'name']
    #change column order
    gene_info_df = gene_info_df[['gene_guidename','strand','length', 'new_sequence', 'name', 'oldname','Benchling_position','distance_from_ATG_bp',
                                 'PAM', 'specificity_score_2013','specificity_score_2016','efficiency_score']]
    #save the df
...
ARF18_guides_present2,ARF18pairs2,ARF18info2 =select_guides(ARF18_guides_file,region_start=22887459,region_end=22888109,lower_bound_distance=90,upper_bound_distance=149)
...
# %%
ARF9adapted_primers = adapt_primers(ARF9info2,'ARF9','../../data/CRISPR_library/sgRNAs-ARF9_new.csv')

# %%
ARF18adapted_primers = adapt_primers(ARF18info2,'ARF18','../../data/CRISPR_library/sgRNAs-ARF18_new.csv')

# %%
DREB26adapted_primers = adapt_primers(DREB26info2,'DREB26','../../data/CRISPR_library/sgRNAs-DREB26_new.csv')

# %%
NLP7adapted_primers = adapt_primers(NLP7info2,'NLP7','../../data/CRISPR_library/sgRNAs-NLP7_new.csv')
...
rename_guidepairs(ARF9pairs2,'ARF9','../../data/CRISPR_library/ARF9guidepairs.csv')
rename_guidepairs(ARF18pairs2,'ARF18','../../data/CRISPR_library/ARF18guidepairs.csv')
rename_guidepairs(DREB26pairs2,'DREB26','../../data/CRISPR_library/DREB26guidepairs.csv')
rename_guidepairs(NLP7pairs2,'NLP7','../../data/CRISPR_library/NLP7guidepairs.csv')

# %%
#output all_guides.csv
#concatenate the primer dfs for each gene
cat = pd.concat([ARF9adapted_primers,ARF18adapted_primers, DREB26adapted_primers, NLP7adapted_primers])
#reset index
...
cat['IDT_name'] = ''
#rename columns
cat = cat.rename(columns={'index': 'Name', 'gene_guidename':'Description','strand':'Strand','new_sequence':'Sequence'})
