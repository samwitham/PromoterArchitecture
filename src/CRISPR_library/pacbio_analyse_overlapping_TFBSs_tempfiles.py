...
def find_overlapping_TFBSs(mutations_df_chunk,mapped_motifs_bed,mapped_motifs_bed_columns,temp_location):
...
def chunkify(mutations_df,mapped_motifs_df,output_folder,gene):
    ...
    for index_slice in index_slices:
        ...
        new_chunk = find_overlapping_TFBSs(chunk,mapped_motifs_bed,mapped_motifs_bed_columns,output_folder)
        chunks.append(new_chunk)

    #concatenate chunks into mutations_df
    mutations_df = pd.concat(chunks)            
    #write out mutations_df
    mutations_df.to_csv(f'{output_folder}{gene}/{gene}_TFBSoverlapping.tsv', sep="\t", index=False, header=1)
...
def main(args):
    ...
    #read in files
    mutations_df,mapped_motifs_df,guide_pairs_df =  read_in_files(args.mutations_tsv, args.mapped_motifs_bed, args.guide_pairs)
    ...
    mutations_df_overlapping_TFBS = chunkify(mutations_df,mapped_motifs_df,args.output_folder,args.gene_name,)
