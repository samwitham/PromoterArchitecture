...
def find_guide_position_in_gene(gene,cut_site_reference_seq, guide_position,gene_fasta_location):
...
def check_guide(root_dir,output,gene,reference_genomic_positions,plantIDs,gene_fasta_location):
    ...
    for subdir, dirs, files in os.walk(root_dir):
            for filename in fnmatch.filter(files,f"*Alleles_frequency_table_around_{gene}_*.txt"):
                ...
                for index, row in filtered_reads.iterrows():
                    ...
                    cut_site_promoter_position = find_guide_position_in_gene(gene,Reference_Sequence_no_dashes,ref_length//2,gene_fasta_location)
                   ...
                   # print(ref_length)
                    if insertion_positions == 'NA':
                        ...
                        for i in insertion_positions:
                            site_promoter_position = find_guide_position_in_gene(gene,Reference_Sequence_no_dashes,i,gene_fasta_location)
   ...
   # print(plantID_df)
    ...
    output_df_new.to_csv(f'{output}{gene}/{gene}_includingduplicates.tsv', sep="\t", index=False, header=1)
    ...
    #write out the output_df
    output_df_merged.to_csv(f'{output}{gene}/{gene}_merged.tsv', sep="\t", index=False, header=1)
...
def main(args):
    ...
    reference_genomic_positions = get_reference_promoter_genomic_positions(args.all_genes_reference_promoter_bed,args.all_promoters_bed)
    ...
    check_guide(args.gene_folder,args.output,args.gene_name,reference_genomic_positions,args.plant_IDs,args.reference_gene_fasta)
