...
def concat_csv_recursive(PATH, EXT):
    ...
    csv_files = [
        file
        for path, subdir, fname in os.walk(PATH)
...
def sort_data(df, output_location):
    ...
    df_plant["LR"] = df_plant.apply(
        lambda row: df[(df.parent == row.root) & (df.root_order == 1)].shape[
            0
        ],
        axis=1,
    )
    # make list of first order lateral root ids
    df_plant["LR_ids"] = df_plant.apply(
        lambda row: df[
            (df.parent == row.root) & (df.root_order == 1)
        ].root.tolist(),
        axis=1,
    )
    # for each id in LR_ids, count the number of rows whose parent root in df is the same as the root id
    df_plant["LR_2nd_order"] = df_plant.apply(
        lambda row: df[
            (df.parent.isin(row.LR_ids)) & (df.root_order == 2)
        ].shape[0],
        axis=1,
    )
    # LRL = total lateral root length (all LRs added together - cm). Have separate column for 2nd order lateral roots

    df_plant["LRL_1st_order"] = df_plant.apply(
        lambda row: df[
            (df.parent == row.root) & (df.root_order == 1)
        ].length.sum(),
        axis=1,
    )
    df_plant["LRL_2nd_order"] = df_plant.apply(
        lambda row: df[
            (df.parent.isin(row.LR_ids)) & (df.root_order == 2)
        ].length.sum(),
        axis=1,
    ...
    # save df as tsv file
    df_plant.to_csv(
        f"{output_location}/single_plant_data.tsv", sep="\t", index=False
    )
...
def qqplots(df, variables, sample_name, output_location):
    ...
    for var in variables:
        ...
        try:
            ...
            anova = smf.ols(
                f"{var} ~ genotype*nitrate_concentration + plate", data=df
            ).fit()
            ...
            # save figure
            plt.savefig(
                f"{output_location}/qqplot_{var}_{sample_name}_residuals.svg",
                format="svg",
                bbox_inches="tight",
                transparent=True,
            )
...
def boxplot(df, var, y_label, sample_name, box_pair_p_values, output_location):
    ...
    order = ["1mM", "10mM"]
    fig_args = {
        "x": "nitrate_concentration",
        "y": var,
        "data": df,
        "order": order,
        "dodge": True,
        "hue": "genotype",
        "hue_order": ["col0", sample_name],
    }
    configuration = {
        "test": None,
        "text_format": "star",
    ...
    }  # "pairs":list(box_pairs_significant.keys()),"pvalues":list(box_pairs_significant.values()), 'loc':'inside'
    _ = plt.figure(figsize=(5, 5))

    fig = sns.boxplot(**fig_args, linewidth=2, palette=["white", "grey"])
    fig = sns.swarmplot(
        **fig_args, color="black", palette=["black", "black"], size=4
    )
    ...
    annotator = Annotator(
        fig, pairs, **fig_args, verbose=False, show_non_significant=False
    )  # show_non_significant=False will be added in the next version of statsannotator
    ...
    leg = plt.legend(
        ...
        fontsize=14,
        frameon=False,
    )  # .set_linewidth(2)#,bbox_to_anchor=(0,0.85), loc='best',
    ...
    for var, y_label in variables.items():
        ...
        if table.loc["genotype:nitrate_concentration"]["PR(>F)"] >= 0.05:
            ...
            df_filtered = df_filtered.dropna(
                subset=["genotype", "nitrate_concentration", var]
            ).copy()
            #
            ...
            for x in range(0, (length_samples)):
                if genotypes_unique[x] != "col0":
                    ...
                    df_10mM = df_filtered[
                        df_filtered["nitrate_concentration"] == "10mM"
                    ]
                    df_1mM = df_filtered[
                        df_filtered["nitrate_concentration"] == "1mM"
                    ]
                    var_10mM = pg.homoscedasticity(
                        df_10mM, group="genotype", dv=f"{var}"
                    )  # Levene test
                    var_1mM = pg.homoscedasticity(
                        df_1mM, group="genotype", dv=f"{var}"
                    )
                    ...
                    res.anova_stat(
                        df=df_filtered,
                        res_var=var,
                        anova_model=f"{var}~C(genotype)+C(nitrate_concentration)",
                    )
                    ...
                    res.tukey_hsd(
                        df=df_filtered,
                        res_var=var,
                        xfac_var=["genotype", "nitrate_concentration"],
                        anova_model=f"{var}~C(genotype)+C(nitrate_concentration)",
                    )
                    ...
                    box_pair_p_values[
                        (("10mM", "col0"), ("10mM", genotypes_unique[x]))
                    ] = p_values.loc[
                        (
                            (p_values["group1"] == ("col0", "10mM"))
                            & (
                                p_values["group2"]
                                == (f"{genotypes_unique[x]}", "10mM")
                            )
                        )
                        | (
                            (
        ...
        if table.loc["genotype:nitrate_concentration"]["PR(>F)"] < 0.05:
            ...
            print(
                f"{var}{sample_name} is significant for genotype*nitrate_concentration interaction"
            )
            df_low = df[df.nitrate_concentration == "1mM"]
            df_high = df[df.nitrate_concentration == "10mM"]
            anova_low_nitrate = smf.ols(
                f"{var} ~ genotype + plate", data=df_low
            ).fit()
            anova_high_nitrate = smf.ols(
                f"{var} ~ genotype + plate", data=df_high
            ).fit()
            ...
            for x in range(0, (length_samples)):
                if genotypes_unique[x] != "col0":
                    box_pair_p_values[
                        (("1mM", "col0"), ("1mM", genotypes_unique[x]))
                    ] = p_value_low_nitrate_df.loc["genotype", "PR(>F)"]
                    box_pair_p_values[
                        (("10mM", "col0"), ("10mM", genotypes_unique[x]))
                    ] = p_value_high_nitrate_df.loc["genotype", "PR(>F)"]
        ...
        # first filter df
        boxplot_df = (
            df.filter(items=[no_log_var, "nitrate_concentration", "genotype"])
            .dropna()
            .copy()
        )
        # get column types
...
def analyse_data(df_plant, sample_name, output_location):
    ...
    df_plant.loc[
        df_plant["log_LRL_div_TRL"] == -np.inf, "log_LRL_div_TRL"
    ] = np.nan
    df_plant.loc[
        df_plant["log_LR_2nd_order"] == -np.inf, "log_LR_2nd_order"
    ] = np.nan
    df_plant.loc[
        df_plant["log_LRL_2nd_order"] == -np.inf, "log_LRL_2nd_order"
    ] = np.nan
    ...
    variables_logs_dict = {
        "log_PR": "Primary root length (cm)",
        "log_LR": "Number of lateral roots",
        "log_LRL": "Total lateral root length (cm)",
        "log_ALRL": "Average lateral root length (cm)",
        "log_TRL": "Total root length (cm)",
        "log_LRD": "Lateral root density",
        "log_LRL_div_TRL": "Ratio of lateral root length to\ntotal root length (LRL/TRL)",
    }  # 'log_LR_2nd_order':'Number of second order lateral roots','log_LRL_2nd_order':'Second order lateral root length (cm)'
    ...
    marginal_effects(
        df_plant, variables_logs_dict, sample_name, output_location
    )
    return df_plant
...
def main():
    ...
    for sample_name in df_plant["sample_name"].unique():
        ...
        # analyse dataframe and make plots
        analyse_data(df_sample, sample_name, output_dir)
