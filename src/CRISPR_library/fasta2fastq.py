...
def fasta_2_fastq(fasta):
    ...
    path = Path(fasta).parent
    fastq_path = f"{path}/fastq"
    # make fastq
    with open(
        f"{path}/{removed_extension}.fasta", "r", encoding="utf-8"
    ) as fasta, open(
        f"{fastq_path}/{removed_extension}.fastq", "w", encoding="utf-8"
    ) as fastq:
...
fasta_filenames = glob.glob(f"{input_directory}/*.fasta", recursive=False)
