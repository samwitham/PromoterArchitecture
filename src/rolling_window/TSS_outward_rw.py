...
def parse_args(args):
    ...
    parser.add_argument(
        "five_UTR_windows",
        type=str,
        help="Input location of 5UTR sliding windows",
    )
    parser.add_argument(
        "promoter_5UTR_bed",
        type=str,
        help="Input location of promoter 5UTR bed file to get strand information",
    )
    parser.add_argument(
        "promoter_windows",
        type=str,
        help="Input location of promoter sliding windows",
    )
    parser.add_argument(
        "TSS_outward_windows",
        type=str,
        help="Output location of the promoter + 5UTR sliding window numbers going outwards from the TSS",
    )
    parser.add_argument(
...
def merge_window_files(
    ...
    fiveUTR_merged = pd.merge(
        fiveUTR, promoters_5UTR, on="AGI", how="left", suffixes=("", "_proms")
    )
    ...
    for i, data in fiveUTR_merged.iterrows():
        ...
        if fiveUTR_merged.loc[i, "strand"] == "+":
            ...
            dict1 = {
                "chr": fiveUTR_merged.loc[i, "chr"],
                "start": start,
                "stop": stop,
                "window_number": 0,
                "AGI": fiveUTR_merged.loc[i, "AGI"],
            }
...
def main(args):
    ...
    merge_window_files(
        args.promoter_windows,
        args.five_UTR_windows,
        args.promoter_5UTR_bed,
        args.TSS_outward_windows,
        args.window_size,
        args.step_size,
    )
