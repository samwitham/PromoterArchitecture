...
parser.add_argument('gene_groups_cv', type=str, help='Input location of coefficient of variation gene categories text file')
parser.add_argument('gene_groups_tau', type=str, help='Input location of tau tissue/condition specificity gene categories text file')
parser.add_argument('expression_data', type=str, help='Input location of log2_transformed microarray expression data in triplicates for each condition/tissue')
args = parser.parse_args()


#make directory for the plots to be exported to
dirName = f'../../data/output/{args.file_names}/genes/gene_group_expression'
try:
...
def makeheatmap(gene_type_expression):
   ...
   # gene_type_expression_data_index = gene_type_expression_data.set_index('gene_type')
    ...
    plt.tight_layout()
    #save figure
    f.savefig(f'../../data/output/{args.file_names}/genes/gene_group_expression/gene_group_expression.pdf', format='pdf')   
        


#read in data and format it correctly
gene_type_expression_data_long,gene_type_expression_data = prepare_files(args.expression_data,args.gene_groups_cv, args.gene_groups_tau)
