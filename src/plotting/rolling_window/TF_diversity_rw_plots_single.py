...
parser.add_argument('window_bed', type=str, help='Input location of window bed file')
parser.add_argument('TF_diversity_bed', type=str, help='Input location of TF diversity bed file')
...
def windows_coords(output_prefix,all_combined_df,
    ...
    merged2 = merged2.astype({'start_no_UTR':'float64','stop_no_UTR':'float64',
                              'TSS':'float64','TSS_transformed_Araport11':'float64',f'{variable_of_interest1}':'float64',
                              f'{variable_of_interest2}':'float64',f'{chromatin_tissue_variable}':'float64'})
...
#         f.tight_layout()
    ...
    f.tight_layout()
    #save figure   
    plt.savefig(f'../../data/output/{args.file_names}/rolling_window/{args.foldername_prefix}/plots/{output_prefix}_TF_diversity_{estimator}_sliding_window.pdf', format='pdf')
    #remove plot
    plt.clf()
    return merged2
...
all_combined = add_coverage(all_combined, args.root_chrom_bp_covered,'root_chrom')
#add shoot chromatin coverage data
all_combined = add_coverage(all_combined, args.shoot_chrom_bp_covered,'shoot_chrom')
#add rootshootintersect chromatin coverage data
all_combined = add_coverage(all_combined, args.rootshootintersect_chrom_bp_covered,'rootshootintersect_chrom')

#plot all promoters in genome - median
all_proms = windows_coords('all_proms',all_combined,
                           'Shannon_diversity_TF','TF Shannon diversity',
                            'Shannon_diversity_TF_family','TF family Shannon diversity',
                           args.promoter_bed,args.promoter_no_5UTR,50,args.EPD_TSS_bed,estimator='median')

#add gene type column to all_combined df
all_combined_genetype = add_genetype(all_combined, args.Czechowski_gene_categories)

#plot median
rolling_rootshootintersect = windows_coords(f'{args.author_name}_genetypenocontrol',all_combined_genetype,
                                          'Shannon_diversity_TF','TF Shannon diversity',
                            'Shannon_diversity_TF_family','TF family Shannon diversity',
                                            args.promoter_bed,args.promoter_no_5UTR,
                         50,args.EPD_TSS_bed,includeEPDTSS=False,estimator='median',  genetype=args.variable1_name, genetype2=args.variable2_name,ci=95, n_boot=10000)


#plot with control genetype too
rolling_incl_control = windows_coords(f'{args.author_name}_genetype',all_combined_genetype,'Shannon_diversity_TF','TF Shannon diversity',
                            'Shannon_diversity_TF_family','TF family Shannon diversity',
                                      args.promoter_bed,args.promoter_no_5UTR, 50,args.EPD_TSS_bed,includeEPDTSS=False,estimator='median',
