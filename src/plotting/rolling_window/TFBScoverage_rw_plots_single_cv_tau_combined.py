...
def parse_args(args):
    ...
    parser.add_argument(
        "bp_covered",
        type=str,
        help="Input location of TFBS base pairs covered file",
    )
    parser.add_argument(
        "EPD_TSS_bed",
        type=str,
...
def process_input_files(bp_covered):
    ...
    coverage_df = coverage_df.assign(
        AGI=coverage_df.name.str.extract(r"(.*?)\_")
    )
    coverage_df["window_number"] = coverage_df.name.str.split(
        "_", n=2, expand=True
    )[1]
    ...
    coverage_df = coverage_df[
        coverage_df["window_number"].map(
            coverage_df["window_number"].value_counts()
        )
        > 99
    ]
    return coverage_df
...
#         sns.distplot(dist_plot).get_figure()
...
#         )
#         sns.distplot(
#             df[df.gene_type == genetype3].length, hist=None, label=genetype3
#         )
#         plt.legend()
...
def main(args):
    ...
    coverage_df = add_coverage(
        coverage_df, args.root_chrom_bp_covered, "root_chrom"
    )
    # add shoot chromatin coverage data
    coverage_df = add_coverage(
        coverage_df, args.shoot_chrom_bp_covered, "shoot_chrom"
    )
    ...
    coverage_prom_types_cv = add_genetype(coverage_df, args.cv_gene_categories)
    coverage_prom_types_tau = add_genetype(
        coverage_df, args.tau_gene_categories
    )
    ...
    windows_coords(
        ...
        "percentage_bases_covered",
        "% bp covered",
        coverage_prom_types_cv,
        coverage_prom_types_tau,
        args.promoter_bed,
        args.promoter_no_5UTR,
        50,
        args.EPD_TSS_bed,
