...
#make directory for the plots to be exported to
dirName = f'../../data/output/{args.file_names}/rolling_window/GC_content_rw/plots'
try:
    # Create target Directory
    os.mkdir(dirName)
    print("Directory " , dirName ,  " created") 
...
def windows_coords(output_prefix,variable_of_interest,variable_of_interest_name,variable_of_interest_df,promoter_bed,promoter_no_5UTR,     window_offset,EPD_TSS_bed,includeEPDTSS=False,includechromatin=False,chromatin_tissue_variable='percentage_bases_covered_rootshootintersect_chrom',          chromatin_tissue_variable_name='% open chromatin root and shoot intersect',x_range=False,estimator='median',ci=95, n_boot=10000,                  genetype=False, genetype2=False, genetype3=False):
...
rolling_rootshootintersect = windows_coords('Czechowski_genetypenocontrol','percentage_GC_content','% GC content',GC_prom_types,args.promoter_bed,args.promoter_no_5UTR,
                         50,args.EPD_TSS_bed,includeEPDTSS=False,x_range=1350,estimator='median',  genetype='constitutive', genetype2='variable',ci=95, n_boot=10000)
