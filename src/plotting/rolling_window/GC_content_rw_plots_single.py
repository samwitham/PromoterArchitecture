...
def process_input_files(Czechowski_gene_categories, GC_content_tsv, palette):
...
def windows_coords(
    ...
    if genetype is False:
        ...
        sns.lineplot(
            y=merged2[variable_of_interest],
            x=merged2.position,
        ...
        # variable of interest lineplot
        sns.lineplot(
            y=merged2[merged2.gene_type == genetype][variable_of_interest],
    ...
    #         f.tight_layout()
        ...
        with open(
            f"../../data/output/{file_names}/rolling_window/{foldername_prefix}/plots/number_of_genes_in_each_category.txt",
            "w",
        ) as file:
            file.write(
        ...
        sns.lineplot(
            y=merged2[merged2.gene_type == genetype2][variable_of_interest],
    ...
    #         f.tight_layout()
        ...
        sns.lineplot(
            y=merged2[merged2.gene_type == genetype3][variable_of_interest],
...
def main(args):
    ...
    GC_content = process_input_files(
        args.Czechowski_gene_categories, args.GC_content_tsv, args.palette
    )

    # add root chromatin coverage data to the df
    GC_content = add_coverage(
        GC_content, args.root_chrom_bp_covered, "root_chrom"
    )
    # add shoot chromatin coverage data to the df
    GC_content = add_coverage(
        GC_content, args.shoot_chrom_bp_covered, "shoot_chrom"
    )
