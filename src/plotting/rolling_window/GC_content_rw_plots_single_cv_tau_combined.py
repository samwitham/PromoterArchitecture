...
def parse_args(args):
    ...
    parser.add_argument(
        "GC_content_tsv",
        type=str,
        help="Input location of GC content tsv file",
    )
    parser.add_argument(
        "EPD_TSS_bed",
        type=str,
...
def windows_coords(
    ...
    if genetype_cv is False:
        ...
        sns.lineplot(
            y=df_cv[df_cv.gene_type == genetype_cv][variable_of_interest],
        ...
        # f.set_size_inches(10, 5)
        sns.lineplot(
            y=df_tau[df_tau.gene_type == genetype_tau][variable_of_interest],
    ...
    #         f.tight_layout()
        ...
        sns.lineplot(
            y=df_cv[df_cv.gene_type == genetype2_cv][variable_of_interest],
        ...
        sns.lineplot(
            y=df_tau[df_tau.gene_type == genetype2_tau][variable_of_interest],
    ...
    #         f.tight_layout()
        ...
        sns.lineplot(
            y=df_cv[df_cv.gene_type == genetype3_cv][variable_of_interest],
        ...
        sns.lineplot(
            y=df_tau[df_tau.gene_type == genetype3_tau][variable_of_interest],
    ...
    # save figure
    plt.savefig(
        f"../../data/output/{file_names}/rolling_window/{foldername_prefix}/plots/{output_prefix}_{variable_of_interest}_{estimator}_sliding_window.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    # remove plot
...
#     # save figure
#     plt.savefig(
#         f"../../data/output/{file_names}/rolling_window/{foldername_prefix}/plots/{output_prefix}_promoter_lengths.pdf",
#         format="pdf",
#     )
#     # remove plot
#     plt.clf()
...
def main(args):
    ...
    GC_content_tau = process_input_files(
        args.tau_gene_categories, args.GC_content_tsv
    )

    # add root chromatin coverage data to the df
    GC_content_tau = add_coverage(
        GC_content_tau, args.root_chrom_bp_covered, "root_chrom"
    )
    # add shoot chromatin coverage data to the df
    GC_content_tau = add_coverage(
        GC_content_tau, args.shoot_chrom_bp_covered, "shoot_chrom"
    )
    # add rootshootintersect chromatin coverage data to the df
    GC_content_tau = add_coverage(
        GC_content_tau,
    ...
    GC_prom_types_cv = add_genetype(GC_content_cv, args.cv_gene_categories)
    GC_prom_types_tau = add_genetype(GC_content_tau, args.tau_gene_categories)
    ...
    # plot rolling window including control category
    windows_coords(
        args.file_names,
        args.foldername_prefix,
        "genetype",
    ...
    # # plot lengths
    # GC_prom_types_length = add_genetype(
    #     all_proms, args.Czechowski_gene_categories
    # )
    # plot_length(
    #     GC_prom_types_length,
    #     "Czechowski_genetypenocontrol",
