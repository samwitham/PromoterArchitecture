...
def parse_args(args):
    ...
    parser.add_argument(
        "tf_gene_categories",
        type=str,
        help="Input location of transcription factor gene categories",
    )
    parser.add_argument(
        "all_genes_ranked", type=str, help="Input location of all ranked genes"
    )
    parser.add_argument(
        "promoter_TF",
        type=str,
        help="Input location of mapped TF-promoter target file",
    )
    parser.add_argument(
        "dependent_variable",
        type=str,
        help="Dependent variable name [tau or expression_cv]",
    )
    parser.add_argument(
        "output_folder_name", type=str, help="Output folder name"
    )
    parser.add_argument(
        "variable1_name",
        type=str,
        help="Optional replacement name for 1st variable eg. non-specific",
...
def make_plot(
    ...
    with open(
        f"../../data/output/{file_names}/{output_folder_name}plots/number_of_genes_in_each_category_{dependent_variable}.txt",
        "w",
    ) as file:
        file.write(
...
def main(args):
    ...
    tf_gene_categories_dfs, promoter_TF_df, merged = read_files(
        args.tf_gene_categories, args.promoter_TF
    )
    ...
    Czechowski_means_genetype = merge_genetype(
        Czechowski_means, args.tf_gene_categories
    )
    ...
    make_plot(
        ...
        "gene_type",
        f"mean_{args.dependent_variable}",
        "Gene type",
        f"Promoter target mean {args.dependent_variable}",
        f"{args.author_name}_{args.dependent_variable}_mean",
        "box",
        args.palette,
        args.file_names,
