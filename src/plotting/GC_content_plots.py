...
def make_plot(
    ...
    with open(
        f"../../data/output/{file_names}/{dependent_variable}/{output_folder_name}plots/number_of_genes_in_each_category_{dependent_variable}.txt",
        "w",
    ) as file:
        file.write(
...
def main(args):
    ...
    GC_content_Czechowski_gene_categories = mergeGC_genecategories(
        GC_content_df, args.Czechowski_gene_categories
    )
    ...
    make_plot(
        GC_content_Czechowski_gene_categories,
        "gene_type",
        "percentage_GC_content",
        "Gene type",
        "% GC content",
