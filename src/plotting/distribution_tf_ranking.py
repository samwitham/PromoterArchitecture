...
parser.add_argument('cv_all_tfs', type=str, help='Input file containing all transcription factors with their CV values')
parser.add_argument('tau_all_tfs', type=str, help='Input file containing all transcription factors with their TAU values')
parser.add_argument('cv_tf_gene_categories', type=str, help='Input file containing CV transcription factor categories')
parser.add_argument('tau_tf_gene_categories', type=str, help='Input file containing TAU transcription factor categories')
...
def read_files(cv_all_tfs, tau_all_tfs, cv_tf_gene_categories, tau_tf_gene_categories):
    ...
    cv_tf_gene_categories_df = pd.read_table(cv_tf_gene_categories,sep='\t',header=None)
    tau_tf_gene_categories_df = pd.read_table(tau_tf_gene_categories,sep='\t',header=None)
    cols = ['AGI','gene_type']
    cv_tf_gene_categories_df.columns = cols
    tau_tf_gene_categories_df.columns = cols
    return cv_all_tfs_df,tau_all_tfs_df,cv_tf_gene_categories_df,tau_tf_gene_categories_df
...
def all_prom_distribution(output_prefix,palette_colours,df, x_variable, x_label, df2=pd.DataFrame(),df1_label='', df2_label='', labels=False,
    ...
    #save to file
    if save == True:
        dist_plot_fig.get_figure().savefig(f'../../data/output/{args.file_names}/genes/TFs/plots/{output_prefix}_distribution.pdf', format='pdf')
    plt.clf()
    return dist_plot_fig


cv_all_tfs_df,tau_all_tfs_df,cv_tf_gene_categories_df,tau_tf_gene_categories_df = read_files(args.cv_all_tfs, args.tau_all_tfs, args.cv_tf_gene_categories, args.tau_tf_gene_categories)
#all czechowski CV distribution plot
all_prom_distribution('cv_dist_plot_czechowski','deep',cv_all_tfs_df,'expression_CV', 'Expression CV',save=True)
#all TAU distribution plot
all_prom_distribution('tau_dist_plot_czechowski','tab10_r',tau_all_tfs_df,'tau', 'Tau',save=True)

#plot TAU for top variable and top constitutive genes from CV ranking'
merged = pd.merge(cv_tf_gene_categories_df,tau_all_tfs_df, left_on='AGI', right_on='Gene_ID', how='left')

#plot CV from CV ranking for top tissue_specific and top constitutive genes from CV ranking'

#merge dfs
merged2 = pd.merge(tau_tf_gene_categories_df,cv_all_tfs_df, left_on='AGI', right_on='Gene_ID', how='left')
...
merged_czechowski_cv =pd.merge(cv_tf_gene_categories_df,cv_all_tfs_df, on='AGI', how='left')
...
merged_czechowski_tau =pd.merge(tau_tf_gene_categories_df,tau_all_tfs_df, on='AGI', how='left')
